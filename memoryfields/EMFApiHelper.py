...
class EMFClient:
    ...
    def create_memory_field(self, name):
        """
        Create a new memory field using the internet-accessible API.
        :param name: The name of the memory field.
        :return: Field ID if successful.
        """
        url = f"{self.base_url}/fields/create-memory-field"
        data = {"name": name}

        try:
            response = requests.post(url, json=data, headers=self.headers)
            response.raise_for_status()
            return response.json()  # Expected: {'message': 'Memory field created successfully', 'field_id': 'some-uuid'}
        except requests.exceptions.RequestException as e:
            print(f"Error creating memory field: {e}")
            raise
